...
var aliases = {
	"KL Divergence": ["Kullback-Leibler Divergence","Kullback-Leibler divergence", "KL divergence"],
...
function replaceMatchingTerm(termName) {
	console.log("Replacing " + termName)
...
for (var key in explanations) {
	...
	for (var i = 0; i < num_aliases; i++) {
		replaceMatchingTerm(aliases[key][i]);
	}
	replaceMatchingTerm(key);
} 
